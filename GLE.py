...
class GLE:
            ...
            if len(self.stimuli[s[0]-1].delay) > 1:
                ...
                if GDC1 < self.binCriteria and GDC2 < self.binCriteria:
                    ...
                    if numChannels == 1:
                        ...
                        if GDC2 != 0 and self.discriminate > abs(GDC1/GDC2-1):
                            s4.append(s)
                        else:
                            if GDC1 > GDC2:
                                s1.append(s)
    ...
    def meanspikespertrial(self, filename, mode='spike', notes=''):
        ...
        if mode == 'ratio':
            s.append('yaxis min '+str(round(min_mean))+' max 2')
            s.append('yaxis dsubticks 0.05')
            s.append('yaxis dticks 0.1')
            s.append('xaxis dticks 4')
            s.append('xaxis dsubticks 0.5')
        else:
        ...
        if self.VariableType == 0:
            for d in range(0,len(self.stimuli),2):
                xnames = xnames + ' ' + str(int(self.stimuli[d].dur[self.Channel]))
        if self.VariableType == 1:
            for d in range(0,len(self.stimuli),2):
                xnames = xnames + ' ' + str(int(self.stimuli[d].delay[self.Channel]-self.rasterOffset))

        xplaces = ''
        for xp in range(0,len(self.stimuli),2):
        ...
        s.append('xlabels hei '+str(0.22*self.scale*long_scale))
        ...
        if mode == 'ratio':
            s.append('amove xg(0) yg(0.5)')
            s.append('aline xg('+str(len(self.stimuli))+') yg(0.5)')
            s.append('amove xg(0) yg(0.75)')
            s.append('aline xg('+str(len(self.stimuli))+') yg(0.75)')
            s.append('amove xg(0) yg(1)')
            s.append('aline xg('+str(len(self.stimuli))+') yg(1)')
        if notes != '':
    ...
    def spikeraster(self, filename, notes=''):
        ...
        # What we guess indiscriminable spikes
        if len(s4) > 0:
            FILE = open(filename+'_spikes_4', "w")
            FILE.writelines('\n'.join(s4))
            FILE.close()
        ...
        if len(s3) > 0:
            ...
            s.append('d3 marker dot msize '+str(0.10*self.scale)+' color grey')
        if len(s4) > 0:
            s.append('data "'+filename.rsplit('/')[-1]+'_spikes_4"')
            s.append('d4 marker dot msize '+str(0.10*self.scale)+' color green')
